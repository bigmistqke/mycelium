...
/** @typedef {{url: string, shortCircuit?: boolean, [key: string]: unknown}} NextResolveResult */
 ...
 * @returns {string}
 */
function syntheticUrlFor(htmlPath, locator) {
  let real
  try {
 ...
 * @returns {string | null}
 */
function realHtmlPathFromSyntheticUrl(url) {
 ...
 * @param {(specifier: string, context: ResolveContext) => Promise<NextResolveResult>} nextResolve
 * @returns {Promise<NextResolveResult>}
 */
...
/** @typedef {{format?: string, [key: string]: unknown}} LoadContext */

/** @typedef {{format: string, source?: string | ArrayBuffer | Uint8Array, shortCircuit?: boolean, [key: string]: unknown}} NextLoadResult */
 ...
 */
function findScript(document, locator) {
 ...
 * @param {(url: string, context: LoadContext) => Promise<NextLoadResult>} nextLoad
 * @returns {Promise<NextLoadResult>}
 */
export async function load(url, context, nextLoad) {
  ...
  if (url.startsWith("file://") && url.includes(markerSegment) && url.endsWith(".mjs")) {
    ...
    const script = findScript(/** @type {Document} */ (document), locator)
