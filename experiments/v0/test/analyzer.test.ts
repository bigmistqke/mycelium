...
import { TypeScriptAnalyzer } from "../src/analyzer.ts";
import {
  computeHash,
  computeImplHash,
...
describe("TypeScriptAnalyzer", () => {
  ...
  describe("extractFunctions", () => {
    it("extracts named function declarations", () => {
      analyzer.addSourceFiles(["test/test-fixtures/functions.ts"]);
  ...
  describe("object literal methods", () => {
    it("extracts shorthand methods from assigned objects", () => {
      analyzer.addSourceFiles(["test/test-fixtures/object-literals.ts"]);
      const result = analyzer.analyze("test-commit");
    ...
    it("handles nested scopes correctly for call argument methods", () => {
      ...
      const nestedMethod = result.entities.find(
        (e) =>
          e.name.includes("outerFunction::") &&
          e.name.includes("register<arg:0>.init"),
      );
    ...
    it("differentiates chained call arguments", () => {
      ...
      const methodA = result.entities.find((e) =>
        e.name.includes("curry<arg:0>.a"),
      );
      const methodB = result.entities.find((e) =>
        e.name.includes("next<arg:0>.b"),
      );
  ...
  describe("nested functions", () => {
    it("extracts nested functions with :: separator", () => {
      ...
      const nested = result.entities.find(
        (e) => e.name === "outerFunction::innerFunction",
      );
  ...
  describe("call graph", () => {
    ...
    it("tracks cross-file function calls", () => {
      analyzer.addSourceFiles([
        "test/test-fixtures/cross-file-a.ts",
        "test/test-fixtures/cross-file-b.ts",
      ]);
      const result = analyzer.analyze("test-commit");
      ...
      const crossFileCall = result.relations.find(
        (r) =>
          r.from_id.includes("::caller") &&
          r.to_id.includes("::utilityFunction"),
      );
    ...
    it("tracks mixed same-file and cross-file calls", () => {
      ...
      const localCall = result.relations.find(
        (r) =>
          r.from_id.includes("::mixedCalls") &&
          r.to_id.includes("::localHelper"),
      );
  ...
  describe("module-level variables", () => {
    it("extracts let variables", () => {
      analyzer.addSourceFiles(["test/test-fixtures/variables.ts"]);
      const result = analyzer.analyze("test-commit");
    ...
    it("extracts const variables", () => {
      ...
      const variable = result.entities.find(
        (e) => e.name === "immutableConfig",
      );
      expect(variable).toBeDefined();
      expect(variable?.kind).toBe("variable");
    });
  ...
  describe("side-effect tracking (reads/writes)", () => {
    it("tracks writes to module variables via assignment", () => {
      ...
      const writeRelation = result.relations.find(
        (r) =>
          r.from_id.includes("::incrementCounter") &&
    ...
    it("tracks writes via property mutation", () => {
      ...
      const writeRelation = result.relations.find(
        (r) =>
          r.from_id.includes("::updateSharedState") &&
  ...
  describe("closure variable detection", () => {
    it("extracts closure variables that are mutated by nested functions", () => {
      analyzer.addSourceFiles(["test/test-fixtures/closures.ts"]);
      const result = analyzer.analyze("test-commit");
  ...
  describe("hash computation", () => {
    ...
    it("normalizes whitespace in signature hash", () => {
      ...
      const hash2 = computeSignatureHash(
  ...
  describe("entity metadata", () => {
    it("includes file path in entity", () => {
      ...
      const fn = result.entities.find((e) => e.name === "namedFunction");
      expect(fn?.file_path).toBe("test/test-fixtures/functions.ts");
    });
  ...
  describe("relation metadata", () => {
    ...
    it("call relations have correct from_id and to_id", () => {
      ...
      const callRelation = result.relations.find(
        (r) =>
          r.from_id.includes("::caller") && r.to_id.includes("::namedFunction"),
      );
      expect(callRelation).toBeDefined();
  ...
  describe("class properties", () => {
    it("extracts class properties as entities", () => {
      analyzer.addSourceFiles(["test/test-fixtures/classes.ts"]);
      const result = analyzer.analyze("test-commit");

      const countProp = result.entities.find(
        (e) => e.name === "count" && e.kind === "property",
      );
    ...
    it("extracts readonly properties", () => {
      ...
      const maxCountProp = result.entities.find(
        (e) => e.name === "maxCount" && e.kind === "property",
      );
    ...
    it("tracks writes to this.x via assignment", () => {
      ...
      const writeRelation = result.relations.find(
        (r) =>
          r.from_id.includes("Counter.reset") &&
    ...
    it("tracks writes to this.x via increment/decrement", () => {
      ...
      const writeRelation = result.relations.find(
        (r) =>
          r.from_id.includes("Counter.increment") &&
          r.to_id.includes("Counter.count") &&
          r.kind === "writes",
      );
      expect(writeRelation).toBeDefined();
    ...
    it("tracks reads from this.x", () => {
      ...
      const readRelation = result.relations.find(
        (r) =>
          r.from_id.includes("Counter.getCount") &&
          r.to_id.includes("Counter.count") &&
          r.kind === "reads",
      );
      expect(readRelation).toBeDefined();
    ...
    it("tracks writes to this.x via mutating methods", () => {
      ...
      const writeRelation = result.relations.find(
        (r) =>
          r.from_id.includes("Counter.addItem") &&
  ...
  describe("call arguments and transitive dependencies", () => {
    it("extracts call arguments for simple function calls", () => {
      analyzer.addSourceFiles(["test/test-fixtures/transitive-flow/*.ts"]);
      const result = analyzer.analyze("test-commit");
    ...
    it("tracks return depending on parameter", () => {
      ...
      const returnDep = result.relations.find(
        (r) =>
    ...
    it("tracks result depending on function return", () => {
      ...
      const resultDep = result.relations.find(
        (r) =>
